...
class Solution {
private:
    void dfs(vector<vector<int>>& grid, std::vector<std::pair<int, int>>& dirs, std::pair<long, long>& p, int r, int c) {
        p.first += grid[r][c];
        p.second += 1;
        grid[r][c] = -1;

        for (auto& d : dirs) {
            int next_r = d.first + r;
            int next_c = d.second + c;

            if (next_r >= 0 && next_r < grid.size() && next_c >= 0 && next_c < grid[0].size() && grid[next_r][next_c] != -1) {
                dfs(grid, dirs, p, next_r, next_c);
            }
        }
    }
    long long by_dfs(vector<vector<int>>& grid) {
        int n = grid.size();
        long long sum = 0;
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                ...
                if (grid[i][j] != -1) {
                    //first: score of visited nodes, second: cnt of visited nodes
