...
public:
    vector<int> longestCommonSubsequence(vector<vector<int>>& arrays) {
        ...
        std::sort(arrays.begin(), arrays.end(), [](const auto& v1, const auto& v2) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 1; j < rows; ++j) {
                int pos = std::lower_bound(arrays[j].begin(), arrays[j].end(), val) - arrays[j].begin();

                if (pos < 0 || pos == arrays[j].size() || arrays[j][pos] != val) {
                    ok = false;
                    break;
                }
            }
