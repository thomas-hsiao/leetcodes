...
class Solution {
private:
    int by_dijkstra_alg(int& n, vector<vector<int>>& edges, int& s, vector<int>& marked) {
        std::unordered_set<int> marked_set(marked.begin(), marked.end());
        ...
        while (!pq.empty()) {
            ...
            if (marked_set.find(node) != marked_set.end()) {
                return dist[node];
            }
...
public:
    int minimumDistance(int n, vector<vector<int>>& edges, int s, vector<int>& marked) {
        return by_dijkstra_alg(n, edges, s, marked);
    }
};