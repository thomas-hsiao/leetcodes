...
public:
    int maxBoxesInWarehouse(vector<int>& boxes, vector<int>& warehouse) {
        ...
        std::vector<int> effect_wh(n);

        int min_h = INT_MAX;
        //from left
        for (int i = 0; i < n; ++i) {
            min_h = std::min(warehouse[i], min_h);
            effect_wh[i] = min_h;
        }
        ...
        for (int i = n - 1; i > -1; --i) {
            min_h = std::min(warehouse[i], min_h);
            effect_wh[i] = std::max(effect_wh[i], min_h);
        }

        //sort effective heights of warehouses
        std::sort(effect_wh.begin(), effect_wh.end());
        std::sort(boxes.begin(), boxes.end());

        int cnt = 0;
        int b = 0;
        for (int i = 0; i < n; ++i) {
            if (b < boxes.size() && effect_wh[i] >= boxes[b]) {
                ++b;
                ++cnt;
            }
        }
